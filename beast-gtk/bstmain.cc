...
main_show_release_notes ()
{
  if (BST_GCONFIG (rc_version) != Bse::version())
    {
      ...
      const char *release_notes_contents_tmpl =
        ...
        "<span tag=\"title\">BEAST/BSE __BEAST_INTERNAL_VERSION__ Release Notes</span>"
        "<newline/><newline/>"
        ...
        "</tag-span-markup>";
      const String release_notes_contents = Bse::string_replace (release_notes_contents_tmpl, "__BEAST_INTERNAL_VERSION__", Bse::version());
      ...
      bst_gconfig_set_rc_version (Bse::version().c_str());
...
bst_print_blurb (void)
{
  ...
  printout ("  -p DRIVERCONF           Set the default PCM driver (if rc-file is missing)\n");
  printout ("  --bse-midi-driver DRIVERCONF\n");
  printout ("  -m DRIVERCONF           Set the default MIDI driver (if rc-file is missing)\n");
  ...
  if (!GTK_WIDGET_VISIBLE (beast_splash))
    {
      ...
      bst_splash_update_entity (beast_splash, Bse::string_format (_("BEAST Version %s"), Bse::version()));
