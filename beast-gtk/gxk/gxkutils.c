...
static const gchar*
gxk_url_create_redirect (const gchar    *url,
                         ...
                         const gchar    *cookie)
{
  ...
  gchar *text = g_strdup_printf ("<!DOCTYPE HTML SYSTEM>\n"
                                 "<html><head>\n"
                                 ...
                                 "<hr>\n"
                                 "<address>GxkUrl/%s file redirect</address>\n"
                                 "</body></html>\n",
                                 url, cookie, url_title, url_title, url, url, ver);
  gint w, c, l = strlen (text);
  do
    w = write (fd, text, l);
  ...
  gxk_cleanup_add (60 * 1000, unlink_file_name, tname); /* free tname */
  return tname;
}

gboolean
gxk_url_show_with_cookie (const gchar           *url,
                          const gchar           *url_title,
                          const gchar           *cookie)
{
  const gchar *redirect = gxk_url_create_redirect (url, url_title, cookie);
  if (redirect)
    return gxk_url_show (redirect);
  else
    return gxk_url_show (url);
}
...
G_LOCK_DEFINE_STATIC (cleanup_mutex);
static GSList *cleanup_list = NULL;

static void
gxk_cleanup_exec_Lm (Cleanup *cleanup)
{
  cleanup_list = g_slist_remove (cleanup_list, cleanup);
  g_source_remove (cleanup->id);
  GDestroyNotify handler = cleanup->handler;
  gpointer data = cleanup->data;
  g_free (cleanup);
  G_UNLOCK (cleanup_mutex);
  handler (data);
  G_LOCK (cleanup_mutex);
}

/**
 * Force all cleanup handlers (see gxk_cleanup_add()) to be immediately
 ...
 */
void
gxk_cleanup_force_handlers (void)
{
  G_LOCK (cleanup_mutex);
  while (cleanup_list)
    gxk_cleanup_exec_Lm (cleanup_list->data);
  G_UNLOCK (cleanup_mutex);
}

static gboolean
gxk_cleanup_exec (gpointer data)
{
  G_LOCK (cleanup_mutex);
  gxk_cleanup_exec_Lm (data);
  G_UNLOCK (cleanup_mutex);
  return FALSE;
}
 ...
 * asyncronously (i.e. not from within gxk_cleanup_add()). The cleanup
 * handler will be called as soon as @a timeout_ms has elapsed or
 * gxk_cleanup_force_handlers() is called.
 */
guint
gxk_cleanup_add (guint                   timeout_ms,
                 GDestroyNotify          handler,
                 gpointer                data)
{
  ...
  cleanup->data = data;
  cleanup->id = g_timeout_add (timeout_ms, gxk_cleanup_exec, cleanup);
  G_LOCK (cleanup_mutex);
