...
struct _IFaceData
{
  ...
  GBaseDestroyFunc  vtable_destroy_base;
};
struct _ClassData
{
  CommonData        common;
  ...
  GBaseDestroyFunc  class_destroy_base;
		   ...
		   GTypeFlags   type_flags)
{
  ...
  if (!branch_last || g_bit_storage (branch_last) < g_bit_storage (g_branch_seqnos[ftype]))
    g_type_nodes[ftype] = g_renew (TypeNode*, g_type_nodes[ftype], 1 << g_bit_storage (g_branch_seqnos[ftype]));
		 ...
		 const GTypeInfo      *info)
{
  ...
  if (!(finfo->type_flags & G_TYPE_FLAG_CLASSED) &&
      (info->class_init || info->class_destroy || info->class_data ||
       (!is_interface && (info->class_size || info->base_init || info->base_destroy))))
    {
      if (pnode)
...
{
  if ((info->interface_destroy || info->interface_data) && !info->interface_init)
    {
  ...
  if (node->is_instantiatable) /* carefull, is_instantiatable is also is_classed */
    {
      ...
      data->instance.class_destroy_base = info->base_destroy;
      data->instance.class_init = info->class_init;
      data->instance.class_destroy = info->class_destroy;
  ...
  else if (node->is_classed) /* only classed */
    {
      ...
      data->class.class_destroy_base = info->base_destroy;
      data->class.class_init = info->class_init;
      data->class.class_destroy = info->class_destroy;
			   ...
			   GType     iface_type)
{
  ...
  for (i = 0; i < node->n_ifaces - 1; i++)
    if (entries[i].iface_type > iface_type)
      break;
  g_memmove (entries + i, entries + i + 1, sizeof (entries[0]) * (node->n_ifaces - i - 1));
...
static void
type_iface_vtable_destroy (TypeNode       *iface,
			   TypeNode       *node,
			   GTypeInterface *vtable)
{
  IFaceEntry *entry = type_lookup_iface_entry (node, iface);
  ...
  if (iholder->info->interface_destroy)
    iholder->info->interface_destroy (vtable, iholder->info->interface_data);
...
static void
type_data_destroy_class_ifaces (TypeNode *node)
{
  IFaceEntry *entry;
  guint i;
  ...
  g_message ("destroying interfaces for %sClass `%s'",
  ...
  while (entry)
    {
      type_iface_vtable_destroy (LOOKUP_TYPE_NODE (entry->iface_type), node, entry->vtable);
...
static void
type_data_destroy_class (TypeNode  *node,
			 ClassData *cdata)
{
  ...
  if (cdata->class_destroy)
    cdata->class_destroy (class, (gpointer) cdata->class_data);
  
  /* call all base class destruction functions in descending order
   */
  if (cdata->class_destroy_base)
    cdata->class_destroy_base (class);
  for (bnode = LOOKUP_TYPE_NODE (NODE_PARENT_TYPE (node)); bnode; bnode = LOOKUP_TYPE_NODE (NODE_PARENT_TYPE (bnode)))
    if (bnode->data->class.class_destroy_base)
      bnode->data->class.class_destroy_base (class);
...
type_data_last_unref (GType type)
{
  ...
  if (node->data->common.ref_count > 1)	/* may have been re-referenced meanwhile */
      ...
      if (node->is_classed && node->data->class.class)
	{
	  ...
	  if (node->n_ifaces)
	    type_data_destroy_class_ifaces (node);
	  node->data = NULL;
	  type_data_destroy_class (node, cdata);
	  g_free (cdata);
	}
      else
	{
		  ...
		  gpointer data)
{
  ...
  for (i = 0; i < gdata->n_qdatas - 1; i++)
    if (qdata[i].quark > quark)
      break;
  g_memmove (qdata + i, qdata + i + 1, sizeof (qdata[0]) * (gdata->n_qdatas - i - 1));
  qdata[i].quark = quark;
  qdata[i].data = data;
}
