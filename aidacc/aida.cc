  ...
  for (const String &kv : pairs)
    {
      ...
      if (!mvpair.second)
        continue;
      if (string_match_identifier_tail (kv.substr (0, mvpair.first), valuestring))
        {
...
/// Given a set of joined `enum_typename` enumerators in `valuestring`, compute the resulting value.
int64_t
Introspection::value_from_enumerators (const std::string &enum_typename, const std::string &valuestring)
{
  const StringVector &kvlist = find_type (enum_typename);
  int64_t value = 0;
  // try exact match
  if (find_enum_value (kvlist, valuestring, &value))
    return value;
  // try bit filling
  ...
  if (parts.size() > 1)
    {
      int64_t result = 0;
      int matches = 0;
      for (const std::string &ident : parts)
        if (find_enum_value (kvlist, ident, &value))
          {
            result |= value;
            matches++;
          }
...
{
  return Introspection::value_from_enumerators (enum_typename, valuestring);
}
#endif

#if 0 // FIXME
/// Construct a string representation of `evalue` in `enum_typename`, joining enumerators with `joiner`.
/// If no exact match is found and `joiner` is provided, enumerators are combined to yield `evalue`, as
/// fallback, a numeric representation is chosen.
std::string
Introspection::enum_values_to_string (const std::string &enum_typename, int64_t evalue, const std::string &joiner)
{
  const StringVector &kvlist = find_type (enum_typename);
  std::vector<std::pair<std::string, int64_t> > values; // saved bit state
  // try exact match
  for (const std::string &kv : kvlist)
    {
      const auto mvpair = split_member_at_property (kv.c_str(), "value");
      if (!mvpair.second)
        continue;                                       // not an IDENT.value=123 entry
      size_t consumed = 0;
      const int64_t value = string_to_int (mvpair.second, &consumed);
      if (!consumed)
        continue;                                       // not a parsable value number
      const std::string ident = kv.substr (0, mvpair.first);
      if (evalue == value)
        return ident;                                   // exact match
      if (!joiner.empty())
        values.push_back (std::make_pair (ident, value));
    }
  // try bit filling
  if (!joiner.empty())
    {
...
#if 0 // FIXME : rm prototype
String
enum_values_to_string (const std::string &enum_typename, int64 evalue, const String &joiner)
{
  return Introspection::value_to_enumerators (enum_typename, evalue, joiner);
}
#endif
